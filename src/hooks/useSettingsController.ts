...
export function useProfileController() {
    ...
    const restoreFromDrive = useCallback(async (manualToken?: string): Promise<void> => {
        ...
        try {
            ...
            if (data.rpg) {
                useAppStore.setState({ coins: data.rpg.coins, xp: data.rpg.xp, inventory: data.rpg.inventory || [] });
            }
            if (data.theme) {
                setProfile({ ...profile, theme: data.theme.colors || profile.theme, banner: data.theme.banner || undefined });
            }
            showToast("Backup restaurado!", "success");
        } catch { showToast("Erro ao baixar dados.", "error"); }
        finally { setIsSaving(false); }
    ...
    return {
        ...
        importProfile,
        updateFullTheme, updateSpecificColor, updateBannerImage,
        deleteAccount: async () => {
